...
def charge_feature_coins(user_id, *, feature_code, coins, idempotency_key=None, remark=None):
    charge_coins = max(int(coins or 0), 0)
    if charge_coins <= 0:
        wallet = get_or_create_wallet(user_id, lock=True)
        db.session.flush()
        return wallet, None
    ...
    wallet = get_or_create_wallet(user_id, lock=True)
    if wallet.available_coins < charge_coins:
        raise InsufficientBalanceError('余额不足，无法执行该功能')

    wallet.available_coins -= charge_coins
    wallet.total_consumed_coins += charge_coins
    ledger = create_wallet_ledger(
        wallet,
        change_type='consume',
        available_delta=-charge_coins,
        idempotency_key=idempotency_key,
        remark=remark or f'{feature_code} 功能扣费 {charge_coins} 代币'
    )
    db.session.flush()
